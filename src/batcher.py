...
def batch_documents(documents: list[str], max_batch_size: int = 10, max_tokens: int = 100_000) -> list[list[str]]:
    """
    Batches documents using a greedy bin-packing algorithm, keeping each batch within token and size limits.
    ...
    Returns:
        list[list[str]]: List of batches, where each batch is a list of document strings.
    """
    ...
    for doc, tokens in sorted_docs:
        ...
        for batch in batches:
            ...
            if len(batch_docs) < max_batch_size and (batch_tokens + tokens) <= max_tokens:
